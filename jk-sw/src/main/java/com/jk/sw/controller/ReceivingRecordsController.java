...
public class ReceivingRecordsController extends BaseController {
    ...
    public String allot(Integer id) {
        ReturnBean users = receivingRecordsService.allotUser(id);
        request.setAttribute("allotList", users.getData());
        List<SwUsable> usableList = swUsableService.selectList(new QueryWrapper<SwUsable>().eq("swid", id)
                .eq("usable", SwConst.UsableScope.getValue("只读")).eq("read_set", SwConst.UsableType.getValue("分发")));
        request.setAttribute("usableList", usableList);
        request.setAttribute("id", id);
        return "sw/receivingRecords/allot";
    }
     ...
     */
    @RequiresPermissions("sw:receivingRecords:allot")
    @PostMapping("sw/receivingRecords/allot")
    @ResponseBody
    public ReturnBean allot(Integer [] uids, Integer id) {
        return receivingRecordsService.allot(id, uids);
    }
