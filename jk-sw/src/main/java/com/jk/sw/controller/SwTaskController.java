...
public class SwTaskController extends BaseController {
    ...
    public ReturnBean insert(Integer swid) {
        return swTaskService.insert(swid);
    }
     ...
     */
    @RequiresPermissions("sw:swTask:list")
    @GetMapping("sw/swTask/versionAttachment")
    public String versionAttachment(Integer id) {
        SwTask task = swTaskService.selectById(id);
        String instructions = swTaskService.getInstructionsByTaskId(id);
        request.setAttribute("propose", task.getPropose());
        request.setAttribute("instructions", instructions);

        ReceivingRecords rr = receivingRecordsService.selectById(task.getSwid());
        if (rr.getEditable()== SwConst.IsOrNot.getValue("否").intValue()){
            request.setAttribute("url", task.getAttachment());
        }else {
            request.setAttribute("text", task.getText());
        }
        request.setAttribute("editable", rr.getEditable());
        request.setAttribute("swid", task.getSwid());
