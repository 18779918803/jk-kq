...
<div class="wrapper">
        ...
        <option value="">==请选择==</option>
            <@dic type="" ; dicList>
                <#list dicList as item>
                    <option value="${item.code}" ${((jkDeviceAds.index==item.code)?string("selected",""))!}>${item.name}</option>
                </#list>
            </@dic>
    </select>
                    </div>
    ...
    <input type="hidden" id="imgUrl"  name="imgUrl" value="${jkDeviceAds.imgUrl!}">
    ...
    <!-- 隐藏域 -->
            <input type="hidden" name="id" id="id" value="${jkDeviceAds.id!}">
                        </div>
                    </form>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function doSubmit() {
        krt.ajax({
            type: "POST",
            url: "${basePath}/kq/jkDeviceAds/update",
