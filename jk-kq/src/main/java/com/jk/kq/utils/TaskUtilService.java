    ...
    public Map<String, Integer> isEnd(String instanceId){
        ...
        if (singleResult == null){
            ...
            if (!ObjectUtils.isEmpty(approvalList)){
                ...
                String deleteReason = approvalList.get(approvalList.size() - 1).getDeleteReason();
                if (ObjectUtils.isEmpty(comment) && !ObjectUtils.isEmpty(deleteReason)){
                    result.put("isPass", CommonConst.IS_PASS_SELF);
                }else {
                    if (comment.contains("同意")){
                        result.put("isPass",CommonConst.IS_PASS_YES);
                    }else {
                        result.put("isPass",CommonConst.IS_END_NO);
                    }
                }
            }
        }else {
