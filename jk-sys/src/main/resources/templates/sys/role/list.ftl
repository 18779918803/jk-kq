...
<@footer>
    <script>
        ...
        $(function () {
                    ...
                    {title: "排序", data: "sort"},
                    {
                        ...
                        render: function (data, type, row) {
                            return '<@shiro.hasPermission name="sys:role:see">'
                                ...
                                + '</button>'
                                + '</@shiro.hasPermission>'
                                + '<@shiro.hasPermission name="sys:role:xcxRes">'
                                + '<button class="btn btn-xs btn-success xcxResBtn" rid="' + row.id + '">'
                                + '<i class="fa fa-edit fa-btn"></i>小程序资源权限'
                                + '</button>'
                                + '</@shiro.hasPermission>';
                        }
                    }
            ...
            //小程序资源权限
            $(document).on("click", ".xcxResBtn", function () {
                var roleId = $(this).attr("rid");
                var url = "xcx/xcxRoleRes/selectResList";
                krt.layer.open({
                    type: 2,
                    area: ['800px', '500px'],
                    title: "小程序资源权限",
                    maxmin: false,
                    content: "${basePath}/common/listSelect?url=" + encodeURI(url) + "&roleId=" + roleId,
                    btn: ['确定', '关闭'],
                    yes: function (index, layero) {
                        var resIds = layero.find("iframe")[0].contentWindow.$("#sId").val();
                        var sName = layero.find("iframe")[0].contentWindow.$("#sName").val();
                        //提交
                        krt.ajax({
                            type: "POST",
                            url: "${basePath}/xcx/xcxRoleRes/insert",
                            data: {
                                "roleId": roleId,
                                "resIds": resIds
                            },
