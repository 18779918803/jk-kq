...
//            //dbApply.getApplyUser()
            if(map.get("applyUser")!=null){
                User sysUser2 = userService.selectById(map.get("applyUser").toString());
                if (sysUser2 != null) {
...
//                dbApply.setTaskName("未启动");
    ...
    public ReturnBean insert(DbApply dbApply) {
        ...
        dbApply.setStatus(1);
        dbApply.setLeadDepartment(organService.selectById(dbApply.getLeadDepartmentId()).getName());
        dbApply.setResponsibilityDepartment(organService.selectById(dbApply.getResponsibilityDepartmentId()).getName());
    ...
    public ReturnBean submitApply(Long id)   {
        ...
        boolean admin = ShiroUtils.getSessionUser().isAdmin();
        if(admin){
            return ReturnBean.error("管理员不能提交审批");
        }
