...
import com.jk.activiti.config.ICustomProcessDiagramGenerator;
import com.jk.activiti.config.WorkflowConstants;
...
public class ProcessController extends BaseController {
     ...
     * 加载审批历史弹窗
     */
    @GetMapping("process/historyList/{instanceId}")
...
//    @RequiresPermissions("process:leave:list")
    @PostMapping("process/listHistory")
    @ResponseBody
    public DataTable listHistory(@RequestParam Map para) {
        IPage<HistoricActivity> list = processService.selectHistoryList(para);
        return DataTable.ok(list);
    }

    @RequestMapping(value = "process/read-resource")
...
//                InputStream imageStream = pec.generateDiagram(bpmnModel, "png", executedActivityIdList, flowIds,"宋体","微软雅黑","黑体",null,2.0);
    ...
    @GetMapping("process/processImg/{instanceId}")
    public String processImg(@PathVariable("instanceId") String instanceId, ModelMap mmap) {
        mmap.put("instanceId", instanceId);
        return prefix + "/processImg";
    }

    @PostMapping("process/delegate")
    @ResponseBody
    public ReturnBean delegate(String taskId, String delegateToUser) {
        User user = (User) SecurityUtils.getSubject().getPrincipal();
        processService.delegate(taskId, user.getUsername(), delegateToUser);
        return ReturnBean.ok();
    }

    @PostMapping( "process/cancelApply")
    @ResponseBody
    public ReturnBean cancelApply(String instanceId) {
        processService.cancelApply(instanceId, "用户撤销");
        return ReturnBean.ok();
    }

    @PostMapping( "process/suspendOrActiveApply")
    @ResponseBody
    public ReturnBean suspendOrActiveApply(String instanceId, String suspendState) {
