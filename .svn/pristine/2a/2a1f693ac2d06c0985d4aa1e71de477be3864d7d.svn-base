...
public class ClockController extends BaseController {
    ...
    @Autowired
    private IClockService punchRecordService;
    ...
    @ResponseBody
    public ReturnBean insert(Clock clock) {
        punchRecordService.insert(clock);
    ...
    public String update(Integer id, String name) {
        Clock clock = punchRecordService.selectById(id);
        request.setAttribute("punchRecord", clock);
    ...
    @ResponseBody
    public ReturnBean update(Clock clock) {
        punchRecordService.updateById(clock);
        return ReturnBean.ok();
    }
