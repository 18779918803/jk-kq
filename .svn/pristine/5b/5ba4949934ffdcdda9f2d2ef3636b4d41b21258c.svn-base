                        ...
                        "render": function (data, type, row) {
                            ...
                            if (row.taskName.indexOf('审批') !== -1) {
                                res += '<@shiro.hasPermission name="Leave:leave:list">'
                                    + '<button class="btn btn-xs btn-success  approvalBtn" taskId="' + row.taskId +'" taskName="' + row.taskName+'">'
                                    + '<i class="fa fa-trash fa-btn"></i>审批'
                                    + '</button>'
                                    + '</@shiro.hasPermission>';
                            } else {
                                if (row.taskName.indexOf('已挂起') === -1) {
                                    res += '<@shiro.hasPermission name="Leave:leave:list">'
                                        + '<button class="btn btn-xs btn-success taskBtn" taskId="' + row.taskId +'" taskName="' + row.taskName+'">'
                                        + '<i class="fa fa-trash fa-btn"></i>' + row.taskName
                                        + '</button>'
                                        + '</@shiro.hasPermission>';
                                }
                            }
