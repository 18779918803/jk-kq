...
public class DbApplyServiceImpl extends BaseServiceImpl<DbApplyMapper, DbApply> implements IDbApplyService {
    ...
    @Transactional
    @Override
    public ProcessInstance submitApply(DbApply entity, String applyUserId, String key, Map objectObjectHashMap)   {
    ...
    public List<DbApply> findTodoTasks(String username, Map param) {
        ...
        for (Task task : tasks) {
            ...
            User user = userService.selectById(dbApply2.getApplyUser());
    ...
    public void approval(Integer id, User user, HttpServletRequest request) {
        ...
        if (!CollectionUtils.isEmpty(taskList)) {
              task = (TaskEntityImpl) taskList.get(0);
        }
        ...
        processService.complete(task.getId(), dbApply.getInstanceId(), "建控集团督察督办事项立项",dbApply.getDbContentBrief()
    ...
    public void reject(Integer id, User user, HttpServletRequest request) {
        ...
        if (!CollectionUtils.isEmpty(taskList)) {
            ...
            param.put("comment","");
            param.put("userId",user.getId());
            processService.complete(task.getId(), dbApply.getInstanceId(), dbApply.getDbContentBrief(), dbApply.getDbSource()
                    , TypeEnums.DBAPPLY.name().toLowerCase()
                    , param, request);
        }
    }
