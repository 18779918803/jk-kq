...
//        return checkList;
    ...
    public void cancel(Integer id, User user, HttpServletRequest request) {
        OvertimeRecord overtimeRecord = overtimeRecordMapper.selectById(id);
        if (overtimeRecord.getIsEnd().equals(CommonConst.IS_END_YES)){
            throw new KrtException(ReturnBean.error("已经撤销"));
        }
        processService.cancelApply(overtimeRecord.getInstanceId(),"用户撤销");
        overtimeRecord.setIsEnd(CommonConst.IS_END_YES);
        overtimeRecord.setIsEnd(CommonConst.IS_PASS_SELF);
        overtimeRecordMapper.updateById(overtimeRecord);
    }
}
